...
static unsigned int mFrameSize;
static int mImgWidth = 0;
static int mImgHight= 0;

unsigned char *mpYlinearbuf_0;     //      global  
unsigned char *mpClinearbuf_0;     //      global 

//========== MFC Decoder / Encoder Color Mode ===================================================
#define NV21_ENCODE                     0       // 0: NV12 Tile Encoding        1: NV21 Linear Encoding
#define NV12LINEAR_DECODE          0      // 0: NV12 Tile Decoding        1: NV12 Linear Decoding

//==========================================================================================
#define S3C_SPI_DMABUF_LEN      (2*1024*1024)

#if     NV21_ENCODE
static struct s3c2410_dma_client s3c_mfc_dma_client = {
	.name = "s3c-mfc-dma",
};

void mfc_dma_NV12_To_NV21_done(void);
static int mfc_dma_NV12_To_NV21_request(mfc_enc_exe_arg *enc_arg);

#if NV12LINEAR_DECODE
static int mfc_convert_NV12Tile_To_NV12Linear(mfc_dec_exe_arg_t *dec_arg);
#endif
...
static unsigned char *mpLinearbuf;
...
/*
  *
  *     NV21 to NV12 color conversion with DMA.
  *
*/
#if NV12LINEAR_DECODE
static int mfc_convert_NV12Tile_To_NV12Linear(mfc_dec_exe_arg_t *dec_arg)
{
	unsigned char *mfc_y_vaddr;
	unsigned char *mfc_c_vaddr;

//==============    for dumping decoded image
#if 0
	char fileName[50];
	memset(fileName, 0, 50);
	sprintf(fileName, "/data/dec_in/mfc_in.yuv");			
#endif

	if(dec_arg->out_display_Y_addr != 0x0)
      {
            mfc_debug("mImgWidth = %d, mImgHight = %d \n", mImgWidth, mImgHight);
            mfc_debug("mpYlinearbuf = %x, mpClinearbuf = %x \n", dec_arg->out_display_Y_addr, dec_arg->out_display_C_addr);

            mfc_y_vaddr =phys_to_virt(dec_arg->out_display_Y_addr);
            mfc_c_vaddr =phys_to_virt(dec_arg->out_display_C_addr);

            tile_to_linear_4x2(mpYlinearbuf_0,mfc_y_vaddr, mImgWidth,mImgHight);
            tile_to_linear_4x2(mpClinearbuf_0,mfc_c_vaddr, mImgWidth,mImgHight>>1);
 
//==============    Copy data from Y/C linear buffer to mfc display buffer.
//            memcpy(mfc_y_vaddr, mpYlinearbuf_0, mImgWidth * mImgHight);
//            memcpy(mfc_c_vaddr, mpClinearbuf_0, mImgWidth * (mImgHight>>1));

//            dec_arg->out_display_Y_addr = virt_to_phys(mpYlinearbuf_0);
//            dec_arg->out_display_C_addr = virt_to_phys(mpClinearbuf_0);

//==============    for dumping decoded image
#if 0
            write_file(fileName,mpYlinearbuf_0,mImgWidth*mImgHight);
#endif
    }
...
static void mfc_set_encode_init_param(mfc_inst_ctx *mfc_ctx, mfc_args *args)
{
	...
	mImgWidth = enc_init_mpeg4_arg->in_width;
      mFrameSize = mImgHight*mImgWidth;

	//=============== for Debugging
#if	DEBUG_MAKE_RAW
        if(mpLinearbuf != NULL)
                kfree(mpLinearbuf);
        mpLinearbuf = kmalloc(mFrameSize, GFP_KERNEL);
#endif
...
MFC_ERROR_CODE mfc_init_encode(mfc_inst_ctx *mfc_ctx, mfc_args *args)
{
	...
	mfc_debug_L0("--\n");
        ...
        {
                if(tx_dma_cpu != NULL)
                        dma_free_coherent(NULL, S3C_SPI_DMABUF_LEN,tx_dma_cpu, tx_dma_phys);
        
                tx_dma_cpu = dma_alloc_coherent(NULL, S3C_SPI_DMABUF_LEN, &tx_dma_phys, GFP_KERNEL);
                if(tx_dma_cpu == NULL){
                        mfc_info("Unable to allocate TX DMA buffers\n");
                        return -1;
                }
        }
#endif  /* NV21_ENCODE  */
...
#if     NV21_ENCODE
        /* Color Convert NV12 to NV21   */
        mfc_dma_NV12_To_NV21_request(enc_arg);
#endif
...
//=============================================== For MFC NV12 Linear
	mImgHight = init_arg->out_img_height - (init_arg->out_crop_top_offset + init_arg->out_crop_bottom_offset);
	mImgWidth = init_arg->out_img_width - (init_arg->out_crop_left_offset + init_arg->out_crop_right_offset );
      mFrameSize = mImgHight*mImgWidth;

#if NV12LINEAR_DECODE
    if(mpYlinearbuf_0 != NULL)
    {
        kfree(mpYlinearbuf_0);
        kfree(mpClinearbuf_0);
    }
    mpYlinearbuf_0 = kmalloc(mFrameSize, GFP_KERNEL);
    mpClinearbuf_0 = kmalloc(mImgWidth * (mImgHight >> 1), GFP_KERNEL);
#endif
...
//        mfc_info("out_crop_top_offset = %d, out_crop_bottom_offset = %d, out_crop_left_offset = %d, out_crop_right_offset = %d",
//                dec_arg->out_crop_top_offset, dec_arg->out_crop_bottom_offset, dec_arg->out_crop_left_offset, dec_arg->out_crop_right_offset);
...
//========================================= Color Convert NV12 Tile to NV12 Linear
#if NV12LINEAR_DECODE
        mfc_convert_NV12Tile_To_NV12Linear(dec_arg);
 #endif
//=======================================================================
...
{
        udelay(100);
        mStatus = 1000;         // End of 1 frame color conversion.
}
...
static int mfc_dma_NV12_To_NV21_request(mfc_enc_exe_arg *enc_arg)
{
	...
	mfc_debug("mfc_dma_NV12_To_NV21_request...IN\n");

        if(mStatus == 0)
        {
                ret = s3c2410_dma_request(DMACH_MTOM_0+nChId, &s3c_mfc_dma_client, NULL);
                if(ret!=0){
                        mfc_err("mfc_dma_NV12_To_NV21_request..........Failure(ret=%d), mStart(%d)\n",ret, mStart);
                        ret = -EBUSY;
                        goto err1;
                }
        }
        ...
        {
                mfc_err("Not Alloc.....Fail \n");
                goto err1;
        }

//=================== Copy Input_To_Buffer
        mfc_debug("mfc_dma_NV12_To_NV21_done = %p , nChId = %d, mFrameSize = %d \n",mfc_dma_NV12_To_NV21_done, DMACH_MTOM_0+nChId, mFrameSize);

	s3c2410_dma_set_buffdone_fn(DMACH_MTOM_0+nChId, mfc_dma_NV12_To_NV21_done);
	s3c2410_dma_devconfig(DMACH_MTOM_0+nChId, S3C_DMA_MEM2MEM, (unsigned int)enc_arg->in_CbCr_addr);
	s3c2410_dma_config(DMACH_MTOM_0+nChId, 1);
        s3c2410_dma_setswap(DMACH_MTOM_0+nChId, 2);             /*      Convert CrCb to CbCr    */
        s3c2410_dma_enqueue(DMACH_MTOM_0+nChId, enc_arg, (unsigned int)tx_dma_phys, mFrameSize);               
        s3c2410_dma_ctrl(DMACH_MTOM_0+nChId, S3C2410_DMAOP_START);

        while(1)
        {
                if(mStatus == 1000)
                {
                        mfc_debug("dma free.............Ok\n");
                        enc_arg->in_CbCr_addr = (unsigned int)tx_dma_phys;
                        s3c2410_dma_free(DMACH_MTOM_0 + nChId,  &s3c_mfc_dma_client);
                        mStatus = 0;
                        return 0;
                }
                nCnt++;
                ...
                if(nCnt > 30)           /*      Timeout Threshold Value = 30msec        */
                        return -1;
        }

	mfc_debug("mfc_dma_NV12_To_NV21_request...OUT   (nChId:%d)\n", DMACH_MTOM_0+nChId);
