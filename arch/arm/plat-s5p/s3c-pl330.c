...
int s3c2410_dma_setswap(enum dma_ch id, unsigned int bytes_swap)
{
	struct s3c_pl330_chan *ch;
	unsigned long flags;
	...
	case 0:	/* No Swap */
		...
		break;
	case 2:
		ch->rqcfg.swap = SWAP_2;
		break;
	case 4:
		ch->rqcfg.swap = SWAP_4;
		break;
	case 8:
		ch->rqcfg.swap = SWAP_8;
		break;
	case 16:
		ch->rqcfg.swap = SWAP_16;
		break;
	default:
		spin_unlock_irqrestore(&res_lock, flags);
		dev_err(ch->dmac->pi->dev, "%s:%d Invalid swap request %d!\n",
