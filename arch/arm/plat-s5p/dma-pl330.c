...
void s3c_dma_dump(int dcon_num, int channel)
{
	...
	printk(KERN_INFO "%d dcon_num %d channel : INT enable %lx\n",
		dcon_num, channel, tmp);
		       ...
		       struct s5p_dma_buf *buf)
{
	...
	do {
		...
		if (chan->flags & S3C2410_DMAF_INFINITYLOOP) {
			pl330_DMA_parameters_t tmp_param;
			tmp_param = dma_param;
			tmp_param.mTrSize /= 4;
			for (i =0;i < 4; i++) {
				if (i == 3) {
					tmp_param.mIrqEnable = 1;
					tmp_param.mLastReq = 1;
					tmp_param.mLoop= 1;
				} else {
					tmp_param.mIrqEnable = 1;
					tmp_param.mLastReq = 0;
					tmp_param.mLoop= 0;
				}
				bwJump += setup_DMA_channel(((u8 *)firstbuf->mcptr_cpu)+bwJump,
						    tmp_param, chan->number);
				tmp_param.mBwJump = bwJump;

				if (dma_param.mDirection == S3C2410_DMASRC_MEM)
					tmp_param.mSrcAddr += tmp_param.mTrSize;
				else if (dma_param.mDirection == S3C2410_DMASRC_HW)
					tmp_param.mDstAddr += tmp_param.mTrSize;
			}
		} else {
			bwJump += setup_DMA_channel(((u8 *)firstbuf->mcptr_cpu)+bwJump,
			...
			dma_addr_t data, int size)
{
	...
	if (chan->state == S5P_DMA_RUNNING) {
		if (chan->flags & S3C2410_DMAF_INFINITYLOOP) {
			printk(KERN_INFO "dma CH %d:It's already working in infinity loop\n",
				chan->number);
			return 0;
		}
