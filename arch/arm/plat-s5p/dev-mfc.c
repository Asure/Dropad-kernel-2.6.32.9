...
void __init s3c_mfc_set_platdata(struct s3c_platform_mfc *pd)
{
	...
	if (!npd) {
		...
		npd->buf_phy_base[0] = s3c_get_media_memory_bank(S3C_MDEV_MFC0, 1);
	} else {
#if (defined(CONFIG_VIDEO_SAMSUNG_MEMSIZE_MFC) || defined(CONFIG_VIDEO_SAMSUNG_MEMSIZE_MFC0))
		npd->buf_phy_base[0] = s3c_get_media_memory_bank(S3C_MDEV_MFC0, 1);
		npd->buf_phy_size[0] = s3c_get_media_memsize_bank(S3C_MDEV_MFC0, 1);
#endif
#ifdef CONFIG_VIDEO_SAMSUNG_MEMSIZE_MFC1
		npd->buf_phy_base[1] = s3c_get_media_memory_bank(S3C_MDEV_MFC1, 1);
		npd->buf_phy_size[1] = s3c_get_media_memsize_bank(S3C_MDEV_MFC1, 1);
#endif
